...
{
  int p1  = (x > 0.03979014977812767f);
  int c1  = (x > 0.3893125355243683f);
  int c2  = (x > 0.6427869200706482f);
  int c3  = (x > 0.8614784181118011f);
  int c4  = (x > 0.5016634166240692f);
  int c5  = (x > 0.2035212516784668f);
  int c6  = (x > 0.2920137718319893f);
  int c7  = (x > 0.1202552504837513f);
  int c8  = (x > -0.33967943489551544f);
  int c9  = (x > -0.13791173323988914f);
  int c10 = (x > -0.045525018125772476f);
  int c11 = (x > -0.23460740596055984f);
  int c12 = (x > -0.6106329262256622f);
  int c13 = (x > -0.4599952697753906f);
  int c14 = (x > -0.8480964004993439f);

  // selector macro: returns (a) if (c)==1, else (b)
  #define sel(c, a, b) ((c) * (a) + ((1 - (c)) * (b)))

  // positive branch: outputs in range [8,15]
  int pos = sel(c1,
                sel(c2, 14 + c3, 12 + c4),
                sel(c5, 10 + c6, 8 + c7));
  // negative branch: outputs in range [0,7]
  int neg = sel(c8,
                sel(c9, 6 + c10, 4 + c11),
                sel(c12, 2 + c13, c14));

  // blend the branches without a branch
  return sel(p1, pos, neg);
}
